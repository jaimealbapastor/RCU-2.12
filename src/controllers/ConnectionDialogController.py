'''
ConnectionDialogController.py
The connection dialog allows a user to edit device connection
...
class ConnectionDialogController(controllers.UIController):
    ...
    def __init__(self, model, threadpool):
        ...
        if QCoreApplication.args.autoconnect:
    ...
    def continue_loading(self, success=False):
        ...
        if self.model.config.is_connected():
            ...
            if not QCoreApplication.args.cli:
                self.cec = controllers.ConnectionErrorController(self)
            else:
