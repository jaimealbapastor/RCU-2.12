...
from .display import DisplayRM1, DisplayRM2, DisplayRM2_rm2fb
from .battery import BatteryRM1, BatteryRM2
...
class RCU:
    ...
    def restart_xochitl(self):
        if self.display:
            ...
            self.display.invalid_cache = True
    ...
    def load_device_info(self):
        ...
        # debug
        #self.device_info['model'] = 'RM110'
        
        # Once we know the model, we can set the device-specific
        # attributes.
        if 'RM100' == self.device_info['model'] \
           or 'RM102' == self.device_info['model']:
            self.display = DisplayRM1(self)
            self.battery = BatteryRM1(self)
        elif 'RM110' == self.device_info['model']:
            # Are hacks applied (using rm2fb)? There should be the
            # existence of a new framebuffer.
            if DisplayRM2_rm2fb.applies(self):
                self.display = DisplayRM2_rm2fb(self)
            else:
                self.display = DisplayRM2(self)
            self.battery = BatteryRM2(self)
        else:
        ...
        out, err = self.run_cmd('grep "usertoken" $HOME/.config/remarkable/xochitl.conf')
        out = out.strip('\n')
        if len(out):
    ...
    def load_notebooks(self, trigger_ui=True, force=False):
        ...
        for uuid in gathered_ids:
            ...
            if 'CollectionType' == metadata['type']:
                ...
                collection.content_dict = content
